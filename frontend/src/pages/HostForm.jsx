...
function WeddingForm() {
  ...
  // Simulating getting hostId from logged-in user (for example, from JWT token or user session)
  const hostId = "6706dabf3e2fc33ef609a4ec"; // Replace with actual hostId from user session or authentication context
  ...
  const handleSubmit = async (e) => {
    ...
    // Include hostId in the formData before submitting
    const weddingData = {
      ...formData,
      hostId, // Add the hostId to the data being sent
    };

    try {
      const response = await axios.post('http://localhost:8000/api/weddings/create', weddingData, {
        headers: {
                ...
                <Card style={{ borderRadius: '8px', boxShadow: '0px 3px 6px rgba(0, 0, 0, 0.1)' }}>
                  <CardContent>
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={6}>
                        <FormControl fullWidth size="small">
                          <InputLabel>Music Offered</InputLabel>
                          <Select
                            name="music"
                            value={formData[day].music}
                            onChange={(e) => handleDayChange(day, e)}
                          >
                            <MenuItem value="yes">Yes</MenuItem>
                            <MenuItem value="no">No</MenuItem>
